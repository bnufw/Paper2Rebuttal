...
allowed-tools: Bash(*), Read, Grep, Glob, Write, Edit, Agent, Skill, mcp__gemini-review__review_start, mcp__gemini-review__review_status
---
...
- **REVIEWER_MODEL = `gemini-review`** — Gemini reviewer invoked through the local `gemini-review` MCP bridge for internal stress-testing. Set `GEMINI_REVIEW_MODEL` if a specific Gemini model is needed.
- **MAX_INTERNAL_DRAFT_ROUNDS = 2** — draft → lint → revise.
- **MAX_STRESS_TEST_ROUNDS = 1** — One Gemini MCP critique round.
...
### Phase 6: Gemini MCP Stress Test
...
After this start call, immediately save the returned `jobId` and poll `mcp__gemini-review__review_status` with a bounded `waitSeconds` until `done=true`. Treat the completed status payload's `response` as the stress-test output, and save the full completed payload to `rebuttal/MCP_STRESS_TEST.md`. If a hard safety blocker appears, revise before finalizing.
...
5. Re-run Gemini MCP stress testing if continuity is useful
